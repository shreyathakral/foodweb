...
function App() {
  ...
  const onDelete=(food)=>{
    ...
    setFoods(foods.filter((e)=>{
      return e!==food;
    })); 

    localStorage.setItem("foods", JSON.stringify(foods));
  }
  const addFood=(title,desc,inputprotein,inputcarbs,inputfats,others)=>{
    console.log("Adding food List",title,desc,inputprotein,inputcarbs,inputfats,others)
    let sno;
    if(foods.length===0)
    {
      sno=0;
    ...
    <Header/> 
      ...
      <Switch>
          <Route exact path="/" render={()=>{
            return(
            <>
            <Foods foods={foods} onDelete={onDelete} buttonsview={"false"}/>
            </>)
          }}> 
          </Route>
